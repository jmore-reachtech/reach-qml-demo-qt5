...
Canvas{
    ...
    property bool drawBlock: false
    property bool showScopeChart: true
    ...
    property bool datasetFill: false
    ...
    onPaint: {
        if (!completed)
        {
            ...
            if (showDot)
            {
                var dotSrc = "import QtQuick 2.0; Canvas {id:dot; z:300; anchors.fill: parent}";
                dot = Qt.createQmlObject(dotSrc, graph, "canvasObject1");
            }
    ...
    function valueChanged(ctx, pointData, currentValue, linePenColor, lastX, lastY, clear)
    {
        ...
        else if (pointData.count < xScale)
        {
            ...
            if (blankOut && clear)
            {
                ...
                {
                    ctx.fillStyle = scaleBgColor;
                    ctx.fillRect(xPos(pointData.count-1), xAxisPosY, distance+2, - (calculatedScale.steps * scaleHop) - scaleLineWidth);
                }
                if (scaleShowLabels)
                    updateTime(ctx);
            }

            var draw = drawLine(ctx, lastX, lastY, linePenColor, pointData);

            lastX = draw.lastX;
            lastY = draw.lastY;
        }
        else if (showScopeChart) {
            var d = {'value': currentValue};
            pointData.append(d);
            var draw = drawLine(ctx, lastX, lastY, linePenColor, pointData);
            ...
            drawWaveFormLine1(graph.getContext("2d"), yAxisPosX + valueHop, topY - scaleLineWidth, xPos(pointData1.count-1) - xPos(1), calculatedScale.steps * scaleHop,  yAxisPosX, topY - scaleLineWidth,
                              ...
                              xPos(pointData1.count-2), yPos(pointData1.count-2, pointData1)+0.5, xPos(pointData1.count-1), yPos(pointData1.count-1, pointData1)+0.5, drawBlock);
    ...
    function drawWaveFormLine1(ctx, sx, sy, sw, sh, dx, dy,
                                        ...
                                        drawBlock)
    {
        var copy = ctx.getImageData(sx.toFixed(1), sy.toFixed(1), sw.toFixed(1), sh.toFixed(1));
        ...
        if (datasetFill){
            ctx.lineTo(x2, xAxisPosY);
            ctx.lineTo(x1, xAxisPosY);
            ctx.lineTo(x1, y1);
            ctx.closePath();
        ...
        if (showDot)
        {
            ...
            dotCtx.strokeStyle = dotColor;
            dotCtx.lineWidth = datasetStrokeWidth;
            dotCtx.arc(x2, y2, dotRadius, 0, Math.PI*2,true);
            dotCtx.fill();
            dotCtx.stroke();
        }
    ...
    function drawLine(ctx, lastX, lastY, linePenColor, pointData)
    {
        ...
        var lastXCalc = xPos(pointData.count-1);
        var lastYCalc = yPos(pointData.count-1, pointData);

        if (bezierCurve && pointData.count > 1)
            ctx.bezierCurveTo(xPos(pointData.count-1.5), yPos(pointData.count-2, pointData), xPos(pointData.count-1.5), lastYCalc, lastXCalc, lastYCalc);
        else
            ctx.lineTo(lastXCalc,lastYCalc);

        ctx.stroke();

        if (datasetFill){
            ctx.lineTo(lastXCalc, xAxisPosY);
            ctx.lineTo(xPos(pointData.count-2), xAxisPosY);
            ctx.lineTo(xPos(pointData.count-2), yPos(pointData.count-2, pointData));
            ctx.closePath();
            ctx.fillStyle = "rgba(0,184,255,0.5)";
            ctx.fill();
        }
        ...
        if (showDot)
        {
            ...
            dotCtx.strokeStyle = dotColor;
            dotCtx.lineWidth = datasetStrokeWidth;
            dotCtx.arc(lastXCalc, lastYCalc, dotRadius, 0, Math.PI*2,true);
            dotCtx.fill();
            dotCtx.stroke();
        }
    ...
    onCurrentValue1Changed: {
        if (currentValue1 !== maxYValue+1)
        {
            graph.requestPaint();
            var data = valueChanged(graph.getContext("2d"), pointData1, currentValue1, line1PenColor, lastX1, lastY1, true)
            lastX1 = data.lastX;
            lastY1 = data.lastY;
    ...
    onCurrentValue2Changed: {
        ...
        {
            var data = valueChanged(graph.getContext("2d"), pointData2, currentValue2, line2PenColor, lastX2, lastY2, false)
            lastX2 = data.lastX;
            lastY2 = data.lastY;
            //To force the onChange event in the future
            currentValue2 = maxYValue + 1;
    ...
    onCurrentValue3Changed: {
        ...
        {
            var data = valueChanged(graph.getContext("2d"), pointData3, currentValue3, line3PenColor, lastX3, lastY3, false)
            lastX3 = data.lastX;
            lastY3 = data.lastY;
            //To force the onChange event in the future
            currentValue3 = maxYValue + 1;
    ...
    onCurrentValue4Changed: {
        ...
        {
            var data = valueChanged(graph.getContext("2d"), pointData4, currentValue4, line4PenColor, lastX4, lastY4, false)
            lastX4 = data.lastX;
            lastY4 = data.lastY;
            //To force the onChange event in the future
            currentValue4 = maxYValue + 1;
    ...
    function clearPens()
    {
        ...
        if (typeof(overlay) !== 'undefined')
            draw(graph.getContext("2d"), overlay.getContext("2d"));
        else
            draw(graph.getContext("2d"), overlay);
        pointData1.clear();
