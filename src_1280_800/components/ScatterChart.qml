...
Canvas {
    ...
    function draw(ctx)
    {
        ...
        function calculateDrawingSizes(){
            ...
            for (var i=0; i<xLabels.length; i++){
                /* updated */
                var textLength = ctx.measureText(xLabels[i]).width;
    ...
    function drawPoint(ctx, x, y, linePenColor) {
    ...
    onCurrentValue1Changed: {
        if (completed && currentValue1 !== (maxY+1).toString())
        {
            ...
            if (values.length > 1)
            {
                ...
                {
                    console.debug("Error: x value is not in range: " + values[0]);
                    return;
                }

                if (parseFloat(values[1]) > maxY || parseFloat(values[1]) < minY)
                {
                    console.debug("Error: y value is not in range: " + values[1]);
                    return;
                }
                ...
                {                   
                    pointData1.append(d);
                    var draw = drawLine(graph.context, lastX1, lastY1, line1PenColor, pointData1);
                    lastX1 = draw.lastX;
                    lastY1 = draw.lastY;
                }

                if (drawPoints)
                    drawPoint(graph.context, lastX1, lastY1, line1PenColor)
            }
    ...
    onCurrentValue2Changed: {
        if (completed && currentValue2 !== (maxY+1).toString())
        {
            ...
            if (values.length > 1)
            {
                ...
                {
                    pointData2.append(d);
                    var draw = drawLine(graph.context, lastX2, lastY2, line2PenColor, pointData2);
                    lastX2 = draw.lastX;
                    lastY2 = draw.lastY;
                }
                if (drawPoints)
                    drawPoint(graph.context, lastX2, lastY2, line2PenColor)
            }
    ...
    onCurrentValue3Changed: {
        if (completed && currentValue3 !== (maxY+1).toString())
        {
            ...
            if (values.length > 1)
            {
                ...
                {
                    pointData3.append(d);
                    var draw = drawLine(graph.context, lastX3, lastY3, line3PenColor, pointData3);
                    lastX3 = draw.lastX;
                    lastY3 = draw.lastY;
                }
                if (drawPoints)
                    drawPoint(graph.context, lastX3, lastY3, line3PenColor)
            }
    ...
    onCurrentValue4Changed: {
        if (completed && currentValue4 !== (maxY+1).toString())
        {
            ...
            if (values.length > 1)
            {
                ...
                {
                    pointData4.append(d);
                    var draw = drawLine(graph.context, lastX4, lastY4, line4PenColor, pointData4);
                    lastX4 = draw.lastX;
                    lastY4 = draw.lastY;
                }
                if (drawPoints)
                    drawPoint(graph.context, lastX4, lastY4, line4PenColor)
            }
